...
if (testMode)
{
    ...
    // テスト用を登録
    builder.Services.AddTransient<IC2Service, C2Service_Test>(sp => new C2Service_Test(connStr));
}
else
{
    // DB接続文字列はメイン側で取得する
    ...
    // 本物を登録
    builder.Services.AddTransient<IC2Service, C2Service>(sp => new C2Service(connStr));
}
