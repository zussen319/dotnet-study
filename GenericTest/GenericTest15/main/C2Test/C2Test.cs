...
try
{
    ...
    // リクエストデータをconfigから取得
    IEnumerable<C2Request> requests = config.GetSection("param").Get<IEnumerable<C2Request>>() ?? [];
    ...
    if (testMode)
    {
		...
		responseStream = executor.RunAsync<C2Service_Test, C2Request, C2Response>(connStr, requests, ct);
    } else
    {
        // 本番用
        // DB接続文字列をconfigから取得
