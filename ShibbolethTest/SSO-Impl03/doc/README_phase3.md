...
**(d) クライアント側のタイムアウトを延ばす**

サーバー側でブレークポイントを止めてステップ実行していると、**SmartClient 側が応答待ちでタイムアウトする**ことがあります。次の症状が出ます。

```
[2] 引換券を JWT に交換します（POST /api/token/exchange）
    ✗ 通信できませんでした
      応答: 通信エラー: The operation has timed out
```

原因は、`HttpWebRequest` の**既定タイムアウトが100秒**であることです。サーバー側で `TicketStore.Consume()` や `JwtHelper.Issue()` を止めている間に、クライアントの100秒が経過して接続が切れます。**サーバー処理自体は成功しており、引換券・JWT の有効期限とは無関係**です（効いていないのは「1回のリクエストの応答待ち時間」）。

対処は、`Program.cs` の `HttpWebRequest` を生成している **2箇所**（`HttpPostJson` と `HttpGet`）に、DEBUG ビルド限定のタイムアウト延長を入れます。

```csharp
HttpWebRequest req = (HttpWebRequest)WebRequest.Create(url);
#if DEBUG
req.Timeout = 60 * 60 * 1000;          // デバッグ時は 60 分（応答待ち）
req.ReadWriteTimeout = 60 * 60 * 1000; // 読み書き途中の無通信の上限
#endif
```

`Timeout` は接続確立と応答待ちの上限、`ReadWriteTimeout` は送受信の途中で無通信が続いたときの上限です。両方を延ばすと、どの段階でブレークポイントを止めても切れません。**`#if DEBUG` で囲うため、本番（Release）は既定の100秒のまま**です。

> **タイムアウトで切れた後の注意**：サーバー側では引換券が既に `Consume`（消費）されています。同じ引換券で再実行しても「無効か使用済み」で 401 になるため、**ブラウザで新しい引換券を取り直して**ください。
>
> なお、この「サーバーは成功したのにクライアントがタイムアウトする」状況は、実運用でネットワークが極端に遅い環境でも起こり得ます（サーバーは JWT を発行したのに、クライアントは受け取れず引換券だけ消費される）。その場合の利用者向け案内（画面を再読み込みして起動し直す）は、SmartClient のメッセージに既に組み込んであります。
...
| [2] で「通信できませんでした」（証明書・URL 起因） | 証明書が信頼されていない／URL 誤り | ルート CA の信頼を確認。診断で接続先 URL を確認 |
| [2] で「通信できませんでした：The operation has timed out」 | **サーバー側でブレークポイントを止めていてクライアントが100秒でタイムアウト** | DEBUG 限定でクライアントのタイムアウトを延長（上記 (d)）。切れた後は新しい引換券を取り直す |
